...
class AIOrchestrator {
    constructor() {
        ...
        this.DETECTION_COOLDOWN = 30000; // 30 seconds cooldown
    ...
    async detectAvailableTools() {
        ...
        console.log(chalk_1.default.gray('🔍 Detecting available AI tools...'));
        ...
        for (const tool of this.availableTools) {
            ...
            try {
                ...
                tool.available = true;
                console.log(chalk_1.default.green(`✓ ${tool.name} detected`));
            }
            catch (error) {
                ...
                if (error.message && (error.message.includes('529') || error.message.includes('429') ||
                    ...
                    console.log(chalk_1.default.yellow(`  ${tool.name} rate limited - will retry later`));
                    // Don't mark detection as complete if we hit rate limits
                    return this.availableTools.filter(tool => tool.available);
                }
                else {
                    console.log(chalk_1.default.gray(`  ${tool.name} not available`));
                }
            }
        }
    ...
    findBestTool(tools, preferredStrengths) {
        return tools.find(tool => preferredStrengths.some(strength => tool.strengths.includes(strength)));
    }
    findCostEffectiveTool(tools) {
        return tools.reduce((best, current) => current.costPerToken < best.costPerToken ? current : best);
    }
    findBalancedTool(tools) {
    ...
    async generateContextPrompt(sessionContext, request) {
...
*Generated by SuperRez CLI - Cost-optimized AI development assistant*`;
    ...
    async executeAICommand(toolName, prompt) {
...
*Note: This is a mock response from Local AI. For detailed assistance, use actual AI tools.*`;
    ...
    buildContextString(sessionContext) {
        if (!sessionContext) {
    ...
    async getSummary() {
        ...
        let summary = chalk_1.default.cyan.bold('🤖 AI Tools Status\n');
        if (available.length > 0) {
            summary += chalk_1.default.green('\n✅ Available Tools:\n');
            available.forEach(tool => {
                const cost = tool.costPerToken === 0 ? 'FREE' : `$${tool.costPerToken}/1K tokens`;
                summary += `  • ${chalk_1.default.bold(tool.name)} (${cost})\n`;
                summary += `    ${tool.strengths.join(', ')}\n`;
            });
        }
        if (unavailable.length > 0) {
            summary += chalk_1.default.yellow('\n⚠️  Unavailable Tools:\n');
            unavailable.forEach(tool => {
                summary += `  • ${chalk_1.default.dim(tool.name)}\n`;
                summary += `    ${chalk_1.default.dim(tool.setupInstructions)}\n`;
            });
        }
        return summary;
    }
